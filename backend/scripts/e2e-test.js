...
await step('Admin : validation du créateur', async () => {
  ...
  expect(pendingList.status === 200 && (pendingList.data.users || []).every(u => Array.isArray(u.missing)), 'La liste des créateurs en attente doit dire ce qui manque à chacun', pendingList);
...
await step('Prospection : ajout manuel qualifié par l\'IA, filtres, statut groupé, export CSV mailing, import annuaire', async () => {
  ...
  try {
    ...
    expect(same.data.created === 0 && same.data.emailsAdded === 0 && same.data.unchanged === 1 && /sans rien de nouveau/.test(same.data.message), 'Réimporter la même ligne ne change rien et le message le dit en clair', same);
...
await step('Extension Chrome : jeton, lot de tâches, remise, résultats (auteur → profil → fiche, bibliothèque publicitaire), blocage', async () => {
  ...
  try {
    ...
    const r1 = await ext('POST', `/${t.id}/result`, { url: t.input.url, title: 'E2E Ext Créatrice (@e2e.extcreator) • Instagram photos and videos', text: 'e2e.extcreator Vidéo UGC skincare', links: [{ href: 'https://www.instagram.com/moncompte.secondaire/', text: 'Profil' }, { href: 'https://www.instagram.com/explore/', text: 'Explorer' }, { href: 'https://www.instagram.com/e2e.extcreator/', text: 'e2e.extcreator' }], self: 'moncompte.secondaire', meta: { description: '12 likes, 3 comments - e2e.extcreator on September 12, 2026: "Ma routine"' } });
    expect(r1.status === 200 && /@e2e\.extcreator/.test(r1.data.outcome), 'L\'auteur doit être lu depuis la description de la page, jamais le compte connecté', r1);
    const { extractPostAuthor } = await import('../src/services/browserTasks.js');
    expect(extractPostAuthor({ self: 'moncompte.secondaire', title: 'Instagram', links: [{ href: 'https://www.instagram.com/moncompte.secondaire/', text: 'Profil' }] }) === null, 'Une page sans autre profil que le compte connecté ne doit donner aucun auteur', {});
    expect(extractPostAuthor({ title: 'Instagram', links: [{ href: 'https://www.instagram.com/quelquun/', text: 'Profil' }, { href: 'https://www.instagram.com/vraie.autrice/', text: 'vraie.autrice' }] }) === 'https://www.instagram.com/vraie.autrice/', 'Le lien portant le pseudo doit primer sur le lien du menu', {});
