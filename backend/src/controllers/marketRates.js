...
export async function rateCalculator(req, res) {
  try {
    ...
    const base = cache.data.rates[videoType] || { min: 80, max: 150, median: 115, source: 'grid', count: 0 };
    ...
    const exclusivityMonths = q.exclusivity === 'true' || q.exclusivity === '1' ? Math.min(36, Math.max(1, parseInt(q.exclusivityMonths || 6, 10))) : 0;
    const factors = [];
    let f = 1;
