...
export async function qualifyLead(lead, { openNiches = [] } = {}) {
  ...
  if (lead.kind === 'creator') {
    const prompt = `${common}
Profil YouTube trouvé avec le mot-clé « ${lead.keyword} » :
- Nom : ${lead.name}
- Pseudo : ${lead.handle || 'inconnu'}
- Abonnés : ${lead.stats?.subscribers ?? '?'}, vidéos : ${lead.stats?.videos ?? '?'}
...
- message : message privé de 300 caractères maximum, personnalisé (cite un élément concret de sa chaîne), qui présente NeedCreator en une phrase et propose de s'inscrire ; pas d'emoji, pas de lien
