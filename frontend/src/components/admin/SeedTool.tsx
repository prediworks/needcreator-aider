...
const EXAMPLE = `# email ; mot de passe ; entreprise ; SIRET ; site web ; secteur ; nombre de campagnes
marque1@votre-domaine.fr ; MotDePasse123! ; Atelier Lumen ; 35600000000048 ; https://atelier-lumen.fr ; beauté ; 3
marque2@votre-domaine.fr ; MotDePasse123! ; Maison Céréales ; ; ; food ; 2`;
  ...
  return (
    <div className="space-y-6">
      <Card className="p-6">
        ...
        <label className="block text-sm font-medium text-neutral-700 mb-1">Une marque par ligne : email ; mot de passe ; entreprise ; SIRET ; site web ; secteur ; nombre de campagnes</label>
        ...
        <p className="text-xs text-neutral-500 mt-1">Secteurs reconnus : beauté, e-commerce, food, tech, mode, services (autre mot = modèle au hasard). SIRET et site facultatifs. Ligne commençant par # ignorée.</p>
        ...
        {p && (
          <div className="mt-4 text-sm">
            ...
            {p.rows.length > 0 && (
              <table className="w-full text-xs"><thead><tr className="text-left text-neutral-500"><th className="py-1 pr-3">Email</th><th className="py-1 pr-3">Entreprise</th><th className="py-1 pr-3">SIRET</th><th className="py-1 pr-3">Modèle</th><th className="py-1 pr-3">Campagnes</th><th className="py-1">Compte</th></tr></thead>
                <tbody>{p.rows.map((r: any) => <tr key={r.email} className="border-t border-neutral-100"><td className="py-1 pr-3">{r.email}</td><td className="py-1 pr-3">{r.companyName}</td><td className="py-1 pr-3">{r.siret || '—'}</td><td className="py-1 pr-3">{r.template}</td><td className="py-1 pr-3">{r.count}</td><td className="py-1">{r.existing ? `existe (${r.existing})` : 'à créer'}</td></tr>)}</tbody></table>
            )}
