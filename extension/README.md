...
`list_ad_library` (6 scrolls). Any other type is read as a single page.
...
- Reads only: no like, follow, comment, message or form submit. The only scripts injected are `extract.js` (read) and `scroll.js` (scroll down).
