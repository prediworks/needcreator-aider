...
| Widget créateur vérifié | Même carte, « Widget » | Dès que le portfolio est validé : image SVG `/c/<slug>/widget.svg` (nom, niveau, missions, note, mise à jour automatique) et code HTML à coller dans un site ou une bio. |
