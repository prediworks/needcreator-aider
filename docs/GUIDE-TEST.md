...
| Devis clients (créateur) | Tableau de bord ou Mes revenus → « Mes devis clients », `/quotes` | Client hors plateforme : devis + projet de contrat PDF générés (informations administratives requises), envoi par email avec lien `/q/<jeton>`. Le client accepte et paie via NeedCreator (inscription marque pré-remplie ou compte existant) : mission privée créée, montant bloqué, contrat, commission « missions extérieures » (Admin → Commission). Ou « Payé en direct » : sans commission, sans facture par mandat. Refus possible par le client. |
