...
| **Transcription des vidéos** (clé à fournir) | Débloque deux fonctions déjà codées : le contrôle « mention du produit » du score de conformité, et les sous-titres automatiques du pack « vidéo prête à diffuser ». Sans clé, seuls les contrôles techniques tournent. Aujourd'hui : `OPENAI_API_KEY` dans le `.env` du backend (≈ 0,006 $ par minute de vidéo), puis `pm2 reload all`. Alternative moins chère possible : Groq (Whisper hébergé, gratuit à petit volume) ou Mistral (Voxtral), une heure de code pour rendre le fournisseur configurable | S (vous, clé) |
